\documentclass{stex}

\libusepackage{naproche}
\libinput{preamble}

\begin{document}
\begin{smodule}{transfinite-induction.ftl}

\importmodule[libraries/set-theory]{ordering.ftl}

\symdecl*{transfinite induction I}
\symdecl*{transfinite induction II}

\begin{theorem*}[forthel,title=Transfinite Induction I,name=transfinite induction I]
  Let $\Phi$ be a class.
  Assume that for all ordinals $\alpha$ if $\Phi$ contains all $\beta \Elem \alpha$ then $\Phi$ contains $\alpha$.
  Then $\Phi$ contains every ordinal.
\end{theorem*}
\begin{proof}[forthel]
  Define $B \DefEq \CClass{x}{x\text{ is a set and if }x \Elem \Ordinals\text{ then }x \Elem \Phi}$.

  Let us show that for all sets $x$ if $B$ contains every element of $x$ that is a set then $B$ contains $x$.
    Let $x$ be a set.
    Assume that every element of $x$ that is a set is contained in $B$.

    \begin{case}{$x \NotElem \Ordinals$.} \end{case}

    \begin{case}{$x \Elem \Ordinals$.}
      Then $\Phi$ contains all ordinals less than $x$.
      Hence $\Phi$ contains $x$.
      Thus $x \Elem B$.
    \end{case}
  End.

  [prover vampire]
  Hence $B$ contains every set (by \sr{epsilon induction}{$\in$-induction}).
  [prover eprover]
  Thus $\Phi$ contains every ordinal.
\end{proof}

\begin{theorem*}[forthel,title=Transfinite Induction II,name=transfinite induction II]
  Let $\Phi$ be a class.
  (Initial case) Assume that $\Phi$ contains $\Zero$.
  (Successor step) Assume that for all ordinals $\alpha$ if $\alpha \Elem \Phi$ then $\Succ(\alpha) \Elem \Phi$.
  (Limit step) Assume that for all limit ordinals $\lambda$ if every $\alpha \Elem \lambda$ is contained in $\Phi$ then $\lambda \Elem \Phi$.
  Then $\Phi$ contains every ordinal.
\end{theorem*}
\begin{proof}[forthel]
  Let us show that for all ordinals $\alpha$ if $\Phi$ contains all ordinals
  less than $\alpha$ then $\Phi$ contains $\alpha$.
    Let $\alpha$ be an ordinal.
    Then $\alpha \Eq \Zero$ or $\alpha$ is a successor ordinal or $\alpha$ is a
    limit ordinal.
    Assume that $\Phi$ contains all $\beta \Elem \alpha$.

    \begin{case}{$\alpha \Eq \Zero$.} \end{case}

    \begin{case}{$\alpha$ is a successor ordinal.}
      Take an ordinal $\beta$ such that $\alpha \Eq \Succ(\beta)$.
      Then $\beta \Elem \Phi$.
      Hence $\alpha \Elem \Phi$ (by successor step).
    \end{case}

    \begin{case}{$\alpha$ is a limit ordinal.}
      Then $\beta \Elem \Phi$ for all ordinals $\beta$ less than $\alpha$.
      Hence $\alpha \Elem \Phi$ (by limit step).
    \end{case}
  End.

  [prover vampire]
  Thus $\Phi$ contains every ordinal (by \sn{transfinite induction I}).
  [prover eprover]
\end{proof}

\end{smodule}
\end{document}
