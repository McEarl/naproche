\documentclass{stex}

\libusepackage{naproche}
\libusepackage{copyright}
\libinput{preamble}

\title{Cantor's Paradoxes}
\author{Marcel Schütz}
\date{2025}

\begin{document}
\begin{smodule}{cantor-paradox.ftl}

\importmodule[libraries/set-theory]{infinite-cardinals.ftl}
\importmodule[articles]{burali-forti-paradox.ftl}
\importmodule[libraries/set-theory]{successor-cardinals.ftl}
\importmodule[libraries/foundations]{universal-class.ftl}

\symdecl*{Cantor Paradox I}
\symdecl*{Cantor Paradox II}

\maketitle

\noindent \emph{Cantor's First Paradox}, discovered by Georg Cantor in 1897,
states that the collection $\Cardinals$ of all (infinite) cardinal numbers is a
proper class \cite[chapter 156]{Cantor1991}.

\begin{theorem}[forthel,title=Cantor's First Paradox,name=Cantor Paradox I]
  $\Cardinals$ is not a set.
\end{theorem}
\begin{proof}[forthel]
  Suppose that $\Cardinals$ is a set.
  Then $\UnionOver \Cardinals$ is a set.

  Let us show that $\UnionOver \Cardinals$ contains every ordinal.
    Let $\alpha$ be an ordinal.
    Choose an infinite ordinal $\beta$ such that $\beta \Geq \alpha$.
    Choose a cardinal $\kappa$ greater than $\beta$.
    Then $\alpha \Elem \kappa \Elem \Cardinals$.
    Hence $\alpha \Elem \UnionOver \Cardinals$.
  End.

  Therefore $\Ordinals \Subclass \UnionOver \Cardinals$.
  Thus $\Ordinals$ is a set.
  Contradiction.
\end{proof}

\emph{Cantor's Second Paradox} denotes the observation that the collection
$\Sets$ of all set is a proper class.
It was shown by Georg Cantor in 1899 via his famous theorem stating that the
cardinality of any set is strictly smaller than the cardinality of its 
powerset \cite[chapter 163]{Cantor1991}.

\begin{theorem}[forthel,title=Cantor's Second Paradox,name=Cantor Paradox II]
  $\Sets$ is not a set.
\end{theorem}
\begin{proof}[forthel]
  Assume the contrary.
  Then $\Pow(\Sets)$ is a subset of $\Sets$.
  Hence $\Card{\Pow(\Sets)} \Leq \Card{\Sets}$.
  Contradiction.
  Indeed $\Card{x} \Less \Card{\Pow(x)}$ for any set $x$.
\end{proof}

\printbibliography
\printlicense[CcByNcSa]{Marcel Schütz (2024--2025)}

\end{smodule}
\end{document}
