...
space = do
  (tokens, warnings) <- anyCharOfCats [EndOfLineCat, SpaceCat]
...
group p = do
  ...
  (_, warning) <- ifEofAddWarning $ Warning beginGroupPos "Missing \"}\" character"
  if null warning
  then do
...
verbCommand = do
  ...
  -- character:
  ifEofAddWarning . Warning beginDelimiterPos $ "Missing \"" <> Text.singleton delimiterChar <> "\" character"
...
inlineForthel p = do
  ...
  when currentlyInsideForthel $ customFailure $ NestedForthel (tokensPos (fst inlineforthelMacro))
  group $ do
...
catchInvalidGroupEnd = do
  ...
  let warning = Warning pos "Missing \"{\" character"
  return ([], [warning])
...
environment p = do
  ...
  if envNameText == "verbatim"
    then do
      ...
      -- Throw an error if the end of the input is reached:
      ifEofAddWarning . Warning beginEnvPos $ "Missing \"\\end{" <> envNameText <> "}\"."
    ...
    -- parser @p@ (while keeping track of the @forthel@ flag):
      ...
      content <- p
      -- Throw an error if the end of the input is reached:
      (_, warning_1) <- ifEofAddWarning . Warning beginEnvPos $ "Missing \"\\end{" <> envNameText <> "}\"."
      if null warning_1
        then do 
...
catchInvalidEnvEnd = do
  ...
  let warning = Warning pos $ "Missing \"\\begin{" <> envNameText <> "}\"."
  return ([], [warning])
